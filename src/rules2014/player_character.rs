...
impl Character {
    ...
    fn parse_custom_action(&self, c: &CustomAction) -> ComputedCustomAction {
        ...
        let stats_attack_bonus = c
            ...
            .sum::<isize>();
        let attack_bonus_prof = if c.add_prof_to_attack {
            self.proficiency_bonus()
        } else {
            0
        };
        let attack_bonus =
            (c.static_attack_bonus as isize + stats_attack_bonus + attack_bonus_prof).max(0);
        ...
        let stats_damage_bonus = c
            ...
            .map(|v| modifiers.stats.get_stat_type(v))
            .sum::<isize>();
        let damage_bonus_prof = if c.add_prof_to_damage {
            self.proficiency_bonus()
        } else {
            0
        };
        let damage_roll_bonus =
            (c.damage_roll.bonus + stats_damage_bonus + damage_bonus_prof).max(0);
