...
async fn level_10_warlock() {
    ...
    *warlock_spells = try_join_all(spells).await.expect("Couldn't get spells");
    ...
    assert_eq!(
        baroopa.classes[0].spellcasting.as_ref().unwrap().1.len(),
        12,
        "Warlock should have 12 spells known at level 10"
    );
    ...
    assert_eq!(
        spell_actions.len(),
        9,
        "Warlock should have 9 spell actions at level 10"
    );
    assert_eq!(
        spell_actions[0].name.to_lowercase(),
        "eldritch blast",
        "First spell action should be eldritch blast"
    );
    assert_eq!(
        spell_actions[0].damage_roll.to_string(),
        "1d10 Force",
        "Eldritch blast damage roll should be 1d10 Force"
    );

    assert_eq!(
        spell_actions[1].name.to_lowercase(),
        "hellish rebuke",
        "2nd spell action should be hellish rebuke"
    );
    assert_eq!(
        spell_actions[1].damage_roll.to_string(),
        "2d10 Fire",
        "Hellish rebuke damage roll should be 2d10 Fire"
    );
    assert_eq!(
        spell_actions[2].name.to_lowercase(),
        "hellish rebuke",
        "3nd spell action should be hellish rebuke"
    );
    assert_eq!(
        spell_actions[2].damage_roll.to_string(),
        "3d10 Fire",
        "Hellish rebuke damage roll should be 3d10 Fire"
    );

    // pact slots should be 2 at level 10, and level 5
    assert_eq!(
        baroopa.available_pact_slots,
        Some(PactSlots { num: 2, level: 5 }),
        "Pact slots at level 10 did not match expected value"
    );

    // cast eldritch blast 3 times
    for _ in 0..3 {
        let eldritch_blast = spell_actions
            .first()
            .expect("Couldn't get eldritch blast spell attack");
        baroopa.cast(eldritch_blast, None);
    }
    assert_eq!(
        baroopa.available_pact_slots,
        Some(PactSlots { num: 2, level: 5 }),
        "Pact slots after casting eldritch blast 3 times did not match expected value"
    );

    // Casting hellish rebuke at 5th level
    let hellish_rebuke = spell_actions
        .get(5)
        .expect("Couldn't get hellish rebuke spell attack");
    assert_eq!(
        hellish_rebuke.damage_roll.to_string(),
        "6d10 Fire",
    ...
    let blight = spell_actions
        .get(7)
        .expect("Couldn't get blight spell attack");
    assert_eq!(
        blight.damage_roll.to_string(),
        "9d8 Necrotic",
        "Blight damage roll when cast at 5th level did not match expected value"
    );
    baroopa.cast(blight, None);
    assert_eq!(
        baroopa.available_pact_slots,
        Some(PactSlots { num: 0, level: 5 }),
        "Pact slots after casting blight did not match expected value"
    );
    ...
    assert_eq!(
        baroopa.available_pact_slots,
        Some(PactSlots { num: 2, level: 5 }),
        "Pact slots after short rest did not match expected value"
    );
    ...
    assert_eq!(
        baroopa.available_pact_slots,
        Some(PactSlots { num: 2, level: 5 }),
        "Pact slots after long rest did not match expected value"
    );
}
