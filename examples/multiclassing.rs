...
async fn main() {
    ...
    character.level_up(&monk).expect("Failed to level up as monk"); // leveling up as monk to
    // level 2
    assert_eq!(character.level(), 2); // now they're level 2
    assert_eq!(character.classes.len(), 1); // they still only have one class: monk
    
    character.level_up(&rogue).expect("Failed to level up as rogue"); // now they multiclass to rogue
