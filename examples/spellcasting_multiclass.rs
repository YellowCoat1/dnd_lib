...
async fn main() {
    ...
    let cleric_to_prepare = spellcaster
        .num_spells(0)
    ...
    let (cleric_prepared_list, _) = spellcaster
        .prepare_spells(0)
    ...
    let (sorcerer_known_list, _) = spellcaster
        .prepare_spells(1)
        .expect("Sorcerer should be able to learn spells");
    ...
    println!(
        "They have {} first level slots and {} second level slots remaining.",
        first_level_slots, second_level_slots
    );
}
